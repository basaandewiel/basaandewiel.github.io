...
Without these options you get `No sources give to Zephyr libraryconsole`
...
However when I compile the **unchanged** program above with this changed proj.conf file and load it in Renode I got following error:
```
10:09:01.2000 [WARNING] sysbus: [cpu: 0x14146] (tag: 'FICR') ReadDoubleWord from non existing peripheral at 0x10000130, returning 0x0. (2)
10:09:01.2015 [WARNING] sysbus: [cpu: 0x124D8] (tag: 'FICR') ReadDoubleWord from non existing peripheral at 0x10000130, returning 0x0. (4)
10:09:01.2016 [WARNING] sysbus: [cpu: 0x14146] (tag: 'FICR') ReadDoubleWord from non existing peripheral at 0x10000130, returning 0x0.
10:09:01.2016 [WARNING] sysbus: [cpu: 0x12626] (tag: 'FICR') ReadDoubleWord from non existing peripheral at 0x10000130, returning 0x0.
10:09:01.2017 [WARNING] sysbus: [cpu: 0x12650] (tag: 'PSELRESET') ReadDoubleWord from non existing peripheral at 0x10001200, returning 0x0.
10:09:01.2017 [WARNING] sysbus: [cpu: 0x1265C] ReadDoubleWord from non existing peripheral at 0x10001204.
...
10:09:01.2135 [DEBUG] nvic: Read from SHCS register. This is not yet implemented. Returning 0
10:09:01.2135 [DEBUG] nvic: Write to SHCS register. This is not yet implemented. Value written was 0x70000.
```

So my preliminary conclusion is that is seems that working with these virtual sensors is not yet supported by Renode.


## Debug Zephyr on Renode via GDB
It is of course nice that we can run Zephyr program on, by Renode, simulated hardware boards. But for developement we also want to be able to debug programs in a simple manner. So we now look into how we can debug programs running in Renode via gdb.

### Working setup for Arduino nano 33 BLE
This is the most interesting part for me. Renode is emulating en Arduino nano 33 BLE, and running a Zephyr program. End thisZephyr program is being debugged via GDB!
...
* Start gdb, and give following commands
...
@@@gdb> l gives not listing of C program; compiled with symbols?



@@@Renode is also able to emulate two bluetooth devices, and you can see the communication between them.
---
RObot Framework
--
TensorFlow LiteML
renode: i $CWD/scripts/blinky.resc
serveVirtualization 3333
<open localhost:3333>



## Integrate Renode with PlatformIO and Zephyr @@@to be completed@@@
...
## Automated testing with Robot Framework @@@to be completed@@@
